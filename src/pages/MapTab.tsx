...
// - Google Map 위에 정적 박물관 핀과 사용자의 촬영 기록 핀을 표시
// - 최신 사용자 기록은 골드 핀, 나머지는 레드 핀
// - 핀 클릭 시 오버레이 카드 표시, 하단에는 그리드 BottomSheet
      ...
      >
        ...
        {items
          ...
          .map((it) => {
            ...
            const url = isLatest ? GOLD_URL : RED_URL;
            ...
            return (
              <Marker
                ...
                zIndex={isLatest ? 9999 : 1} // 최신 핀을 위로
                onClick={() =>
                  setSelected({
                    kind: "user",
                    ...
                    shotAt: it.shotAt,
                    museumName:
                      it.recognizedWorkId === "kkachi_tiger"
                        ? "호암미술관"
                        : it.museumName ?? "내 위치",
                  })
                }
        ...
        {selected && (
          ...
          >
            <div className="relative -translate-y-4">
              {/* 꼬리 */}
              <div
                className="absolute left-1/2 -translate-x-1/2 top-0 w-0 h-0 border-l-8 border-r-8 border-b-8
                           border-l-transparent border-r-transparent border-b-[#0B1224]/90 drop-shadow"
              />
              {/* 카드 */}
              <div
                className="mt-2 min-w-[220px] max-w-[260px] rounded-xl bg-[#0B1224]/90 text-white shadow-xl
